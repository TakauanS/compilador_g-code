...
class ViewMenRpm(ctk.CTkToplevel):
    ...
    def __init__(self):
        try:
            ...
            self.config(bg=ViewMenRpm.cor1)
            self.resizable(False, False)
            self.grab_set() # Dar foco máximo a tela de posicionamentos 

            # SEÇÃO DE FRAMEs
            fra_p = self.__assents.criar_frame(10, 5, 305, 830, 10, 1, ViewMenRpm.cor1, ViewMenRpm.cor1, self)

            # SEÇÃO DE LABELs
            label_title = self.__assents.criar_label('Configurações - Rotações do Fuso', 10, 5, text_color=ViewMenRpm.cor2, font=('Corbel', 22, 'bold'), frame=fra_p)
            label_sub = self.__assents.criar_label(ViewMenRpm.sub, 10, 35, text_color=ViewMenRpm.cor4, font=('Corbel', 18, 'normal'), frame=fra_p)

            label_mod = self.__assents.criar_label('MODO DE VELO:', 10, 75, 0, 'MODO DE VELOCIDADE DO SPINDLE', text_color=ViewMenRpm.cor4, frame=fra_p)
            label_sen = self.__assents.criar_label('SENTI. DE GIRO :', 10, 111, 0, 'SENTIDO DE GIRO DO SPINDLE', text_color=ViewMenRpm.cor4, frame=fra_p)

            label_fus = self.__assents.criar_label('FUSO :', 520, 75, 0, 'ESCOLHA O FUSO (OPCIONAL)', text_color=ViewMenRpm.cor4, frame=fra_p)
            label_lim = self.__assents.criar_label('Limites - Superiores e Inferiores', 10, 160, text_color=ViewMenRpm.cor2, font=('Corbel', 22, 'bold'), frame=fra_p)

            label_sup = self.__assents.criar_label('SUPERIOR :', 10, 200, 0, 'LIMITE DE ROTAÇÃO SUPERIOR', text_color=ViewMenRpm.cor4, frame=fra_p)
            label_inf = self.__assents.criar_label('INFERIOR  :', 10, 235, 0, 'LIMITE DE ROTAÇÃO SUPERIOR', text_color=ViewMenRpm.cor4, frame=fra_p)

            # SEÇÃO DE LINHAs
            linha_h = ctk.CTkLabel(fra_p, 0, 0, text='', image=ViewMenRpm.img_lh)
